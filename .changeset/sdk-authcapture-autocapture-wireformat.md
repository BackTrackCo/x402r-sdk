...
- `x402rDefaults({ captureAuthorizer, feeRecipient?, captureDeadline?, refundDeadline?, minFeeBps?, maxFeeBps?, name?, version?, autoCapture?, assetTransferMethod? }) → AuthCaptureExtra` from `@x402r/helpers`. Quick-start builder with sensible defaults: `feeRecipient` defaults to `captureAuthorizer` (x402r deployment convention — operator-and-fee-recipient are the same EOA); deadlines default to `now + 24h` / `now + 7d`; fee policy `0`–`100` bps; token domain `USDC` / `2`. Only `captureAuthorizer` is required. Per-field JSDoc warns on the production footguns: `maxFeeBps: 100` reverts if facilitator charges more; `name: 'USDC'` fails EIP-712 verification for other tokens; `captureDeadline` default favors async capture pipelines. Optional `autoCapture` / `assetTransferMethod` pass through unchanged so the facilitator's wire-spec defaults take over when unset.
