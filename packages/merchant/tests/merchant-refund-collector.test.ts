...
describe("X402rMerchant - ReceiverRefundCollector", () => {
  ...
  describe("approveRefundBudget", () => {
    it("should call token.approve with collector address and amount", async () => {
      ...
      const result = await merchant.approveRefundBudget(TOKEN_ADDRESS, amount);
    ...
    it("should throw if receiverRefundCollectorAddress is not set", async () => {
      ...
      await expect(merchant.approveRefundBudget(TOKEN_ADDRESS, BigInt("1000"))).rejects.toThrow(
  ...
  describe("getRefundBudget", () => {
    it("should call token.allowance with receiver and collector addresses", async () => {
      ...
      const budget = await merchant.getRefundBudget(TOKEN_ADDRESS);
    ...
    it("should throw if receiverRefundCollectorAddress is not set", async () => {
      ...
      await expect(merchant.getRefundBudget(TOKEN_ADDRESS)).rejects.toThrow(
        "ReceiverRefundCollector address required",
      );
    });
  });
  ...
  describe("refundPostEscrowFromBudget", () => {
    it("should call refundPostEscrow with collector address and empty data", async () => {
      ...
      const result = await merchant.refundPostEscrowFromBudget(paymentInfo, amount);
    ...
    it("should throw if receiverRefundCollectorAddress is not set", async () => {
      ...
      const paymentInfo = createPaymentInfo();
      await expect(
        merchant.refundPostEscrowFromBudget(paymentInfo, BigInt("500000")),
      ).rejects.toThrow("ReceiverRefundCollector address required");
    });
  });

  describe("config", () => {
    it("should expose receiverRefundCollectorAddress", () => {
      const merchant = new X402rMerchant({
        publicClient,
        walletClient,
        operatorAddress: OPERATOR_ADDRESS,
      ...
      expect(merchant.receiverRefundCollectorAddress).toBe(RECEIVER_REFUND_COLLECTOR);
    });
    ...
    it("should default receiverRefundCollectorAddress to undefined", () => {
      ...
      expect(merchant.receiverRefundCollectorAddress).toBeUndefined();
    });
  });
});
